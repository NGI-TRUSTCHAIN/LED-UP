...
import { DidAuth } from '../../../typechain-types';

// Helper function to convert role string to bytes32
async function getRoleHash(role: string, didAuthContract: DidAuth): Promise<string> {
...
async function main() {
  ...
  const didAuthAddress = '0x91c8C745fd156d8624677aa924Cdc1Ef8173C69C';
  const did = 'did:ethr:0xdF3e18d64BC6A983f673Ab319CCaE4f1a57C7097';
  const role = 'CONSUMER';

  // Validate required parameters
  if (!didAuthAddress || !did || !role) {
    console.error(`
Usage: CONTRACT_ADDRESS=<did-auth-address> DID=<did> ROLE=<role> npx hardhat run scripts/did-auth/grant-did-role.ts --network <network>

Example: 
CONTRACT_ADDRESS=0x123...abc DID=did:example:123 ROLE=CONSUMER npx hardhat run scripts/did-auth/grant-did-role.ts --network localhost
`);
    process.exit(1);
  }
  ...
  try {
    ...
    const didAuthContract = await ethers.getContractAt('DidAuth', didAuthAddress, deployer);
    ...
    // Call the grantDidRole function
    console.log(`Granting role '${role}' (${roleHash}) to DID: ${did}...`);
    const tx = await didAuthContract.grantDidRole(did, roleHash);
    console.log(`Transaction sent. Waiting for confirmation...`);
    await tx.wait();

    console.log(`Successfully granted role '${role}' to DID: ${did}`);
    console.log(`Transaction hash: ${tx.hash}`);
    console.log(`DidAuth contract address: ${didAuthAddress}`);
  } catch (error: any) {
    console.error('Error executing grant-did-role script:');
