...
async function main() {
  ...
  if (!didAuthAddress || !did || !role) {
    console.error(`
Usage: CONTRACT_ADDRESS=<did-auth-address> DID=<did> ROLE=<role> npx hardhat run scripts/did-auth/has-did-role.ts --network <network>

Example: 
CONTRACT_ADDRESS=0x123...abc DID=did:example:123 ROLE=CONSUMER npx hardhat run scripts/did-auth/has-did-role.ts --network localhost
`);
    process.exit(1);
  }
  ...
  try {
    ...
    // Call the hasDidRole function
    const hasRole = await didAuthContract.hasDidRole(did, roleHash);
    
    console.log(`DID: ${did}`);
    console.log(`Role: ${role} (${roleHash})`);
    console.log(`Has role: ${hasRole}`);
    
    // Return the result for use in scripts
    return hasRole;
  } catch (error: any) {
    console.error('Error executing has-did-role script:');
