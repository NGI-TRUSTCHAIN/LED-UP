...
import { COMPENSATION_CONTRACT_ADDRESS, PATIENT_PRIVATE_KEY } from '../constants';
...
export const registerProducerRecord = async ({
  ...
  const tx = await DataRegistry.registerProducerRecord(
    recordId,
    producer,
    signature,
    resourceType,
 ...
 * @throws {Error} - Throws an error if the transaction fails.
 */
export const updateProviderRecordSchema = async (
  _schemaRef: RecordSchema
 ...
 * @throws {Error} - Throws an error if the transaction fails.
 */
export const updateProducerConsent = async (
  producer: string,
  status: number
): Promise<TransactionReceipt> => {
 ...
 * @throws {Error} - Throws an error if the transaction fails.
 */
export const updateProducerRecordStatus = async (
  _producer: string,
  _status: number
): Promise<TransactionReceipt> => {
...
export const withdrawProducerBalance = async (amount: number): Promise<TransactionReceipt> => {
  const signer = await new Wallet(PATIENT_PRIVATE_KEY, provider);
 ...
 * @throws {Error} - Throws an error if the withdrawal fails.
 */
export const withdrawServiceFee = async (amount: number): Promise<TransactionReceipt> => {
  const tx = await Compensation.withdrawServiceFees(amount);
  return await tx.wait();
};
