 ...
 */
export const writeDatabase = async (data: BlockchainRecordType): Promise<any> => {
  try {
 ...
 */
export const write = (context: InvocationContext, data: BlockchainRecordType): void => {
  context.extraOutputs.set(sendToSql, JSON.stringify(data));
};

export const readLatestBlocks = async (): Promise<any> => {
  const pool = await poolPromise;
  const result = await pool
    .request()
    .query('SELECT TOP 10 * FROM dbo.DataRegistryEvents ORDER BY eTimestamp DESC');
