/**
 * Azure Function to update a public key in a DID document.
 */
import { app, HttpRequest, HttpResponseInit, InvocationContext } from '@azure/functions';
// import { z } from 'zod';

import { authMiddleware } from '../../helpers/auth-middleware';
import { getContractAbi, getContractAddress } from '../../helpers/contract-config';
import { DidRegistryService, DidResolverService } from '../../services';
import { UserRole } from '../../types/auth-types';
import { UpdatePublicKeyRequest, UpdatePublicKeyResponse } from '../../types/did-types';

// const UpdatePublicKeySchema = z.object({
//   did: z.string(),
//   keyId: z.string(),
//   publicKey: z.string(),
// });

/**
 * Handler for the HTTP function that updates a public key in a DID document.
 *
 * This function processes a POST request to update a public key in a DID document.
 * It returns a 200 OK response with the updated DID document.
 * In case of an error, it returns a 400 Bad Request response.
 *
 * @param {HttpRequest} request - The HTTP request object.
 * @param {InvocationContext} context - The context object providing information about the execution of the function.
 * @returns {Promise<HttpResponseInit>} A promise that resolves to an HttpResponseInit object containing the response data.
 *
 * @example
 * Example Request:
 * POST /did/update-key
 * Body:
 * {
 *   "did": "did:ethr:0x1234567890abcdef1234567890abcdef12345678",
 *   "keyId": "did:ethr:0x1234567890abcdef1234567890abcdef12345678#keys-1",
 *   "publicKey": {
 *     "type": "EcdsaSecp256k1VerificationKey2019",
 *     "controller": "did:ethr:0x1234567890abcdef1234567890abcdef12345678",
 *     "publicKeyJwk": {
 *       "kty": "EC",
 *       "crv": "secp256k1",
 *       "x": "...",
 *       "y": "..."
 *     }
 *   }
 * }
 *
 * Example Response:
 * Status: 200
 * Body:
 * {
 *   "didDocument": {
 *     "@context": [
 *       "https://www.w3.org/ns/did/v1",
 *       "https://w3id.org/security/suites/secp256k1-2019/v1"
 *     ],
 *     "id": "did:ethr:0x1234567890abcdef1234567890abcdef12345678",
 *     "controller": [
 *       "did:ethr:0x1234567890abcdef1234567890abcdef12345678"
 *     ],
 *     "verificationMethod": [
 *       {
 *         "id": "did:ethr:0x1234567890abcdef1234567890abcdef12345678#keys-1",
 *         "type": "EcdsaSecp256k1VerificationKey2019",
 *         "controller": "did:ethr:0x1234567890abcdef1234567890abcdef12345678",
 *         "publicKeyJwk": {
 *           "kty": "EC",
 *           "crv": "secp256k1",
 *           "x": "...",
 *           "y": "..."
 *         }
 *       }
 *     ],
 *     "authentication": [
 *       "did:ethr:0x1234567890abcdef1234567890abcdef12345678#keys-1"
 *     ]
 *   }
 * }
 *
 * Error Responses:
 * Status: 400
 * Body:
 * {
 *   "error": "Bad Request",
 *   "message": "DID is required"
 * }
 */
export async function handler(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  context.log(`Http function processed request for url "${request.url}"`);

  // Check authentication
  const authError = authMiddleware(request, [UserRole.ADMIN]);
  if (authError) {
    return authError;
  }

  try {
    // Parse the request body
    const body = (await request.json()) as UpdatePublicKeyRequest;

    // Validate the request
    if (!body.did) {
      return {
        status: 400,
        jsonBody: {
          error: 'Bad Request',
          message: 'DID is required',
        },
      };
    }

    if (!body.keyId) {
      return {
        status: 400,
        jsonBody: {
          error: 'Bad Request',
          message: 'Key ID is required',
        },
      };
    }

    if (!body.publicKey) {
      return {
        status: 400,
        jsonBody: {
          error: 'Bad Request',
          message: 'Public key is required',
        },
      };
    }

    // Initialize the DidRegistryService
    const didRegistryService = new DidRegistryService(
      getContractAddress('DID_REGISTRY'),
      getContractAbi('DID_REGISTRY')
    );

    // Initialize the DidResolverService
    const didResolverService = new DidResolverService(didRegistryService, undefined, context);

    // Check if the DID is active
    const isActive = await didRegistryService.isDidActive(body.did);

    if (!isActive) {
      return {
        status: 400,
        jsonBody: {
          error: 'Bad Request',
          message: 'DID is not active or does not exist',
        },
      };
    }

    // Resolve the DID
    const result = await didResolverService.resolve(body.did);

    // Check if the DID was resolved successfully
    if (result.didResolutionMetadata.error) {
      return {
        status: 400,
        jsonBody: {
          error: 'Bad Request',
          message: `Failed to resolve DID: ${result.didResolutionMetadata.error}`,
        },
      };
    }

    // Find the verification method to update
    const verificationMethods = result.didDocument.verificationMethod || [];
    const keyIndex = verificationMethods.findIndex(vm => vm.id === body.keyId);

    if (keyIndex === -1) {
      return {
        status: 400,
        jsonBody: {
          error: 'Bad Request',
          message: `Key ID ${body.keyId} not found in DID document`,
        },
      };
    }

    // Update the verification method
    const updatedVerificationMethods = [...verificationMethods];

    updatedVerificationMethods[keyIndex] = {
      ...updatedVerificationMethods[keyIndex],
      ...body.publicKey,
      id: body.keyId, // Ensure the ID remains the same
    };

    // Update the DID document with the new verification method
    const updatedDocument = await didResolverService.updateDidDocument(body.did, {
      verificationMethod: updatedVerificationMethods,
    });

    // Return the updated DID document
    const response: UpdatePublicKeyResponse = {
      didDocument: updatedDocument,
    };

    return {
      status: 200,
      jsonBody: response,
    };
  } catch (error: any) {
    context.error(`Error updating public key: ${error}`);

    return {
      status: 400,
      jsonBody: {
        error: 'Bad Request',
        message: error.message || 'Failed to update public key',
      },
    };
  }
}

/**
 * HTTP route configuration for the Azure Function to update a public key in a DID document.
 */
app.http('updatePublicKey', {
  methods: ['POST'],
  route: 'did/update-public-key',
  authLevel: 'anonymous',
  handler,
});
