/**
 * Azure Function to get a DID document.
 */
...
import { GetDidDocumentRequest, GetDidDocumentResponse } from '../../types/did-types';

/**
 * Handler for the HTTP function that gets a DID document.
 *
 * This function processes a POST request to get a DID document.
 * It returns a 200 OK response with the DID document.
...
export async function handler(
  ...
  try {
    ...
    const body = (await request.json()) as GetDidDocumentRequest;
    ...
    const response: GetDidDocumentResponse = {
    ...
    context.error(`Error getting DID document: ${error}`);

    return {
      status: 400,
      jsonBody: {
        error: 'Bad Request',
        message: error.message || 'Failed to get DID document',
      },
    };
  }
}
...
/**
 * HTTP route configuration for the Azure Function to get a DID document.
 */
app.http('getDidDocument', {
  methods: ['POST', 'GET'],
  route: 'did/document',
  authLevel: 'anonymous',
  handler,
