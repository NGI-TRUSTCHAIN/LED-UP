...
import { OWNER_PRIVATE_KEY, RPC_URL, DID_AUTH_CONTRACT_ADDRESS, DID_REGISTRY_CONTRACT_ADDRESS } from '../constants';
...
export class BaseBlockchainManager {
  ...
  constructor() {
    ...
    if (didRegistryAddress && didRegistryAddress.trim() !== '') {
      this.didRegistryContract = new ethers.Contract(didRegistryAddress, DidRegistryABI, this.signer);
    }
  }
   ...
   */
  protected ensureContractInitialized(contract: ethers.Contract | null, contractName: string): ethers.Contract {
    if (!contract) {
      throw new Error(`${contractName} is not initialized. Please check your environment variables.`);
    }
    return contract;
  }
}
