...
export class DataRegistryErrorHandler extends BaseErrorHandler {
  ...
  protected handleInternalError(error: any): ParsedContractError {
    // Check if this is a DID authentication error
    if (error.message?.toLowerCase().includes('did')) {
      return {
    ...
    // Check if this is a role-based error
    if (error.message?.toLowerCase().includes('role') || error.message?.toLowerCase().includes('access')) {
      return {
        errorName: 'DataRegistry__Unauthorized',
        args: {},
        message:
          'Unauthorized: You do not have permission to perform this action. Please check your role and permissions.',
        originalError: error,
      };
    }

    // Check if this is a paused contract error
    if (error.message?.toLowerCase().includes('pause')) {
      return {
        errorName: 'DataRegistry__ServicePaused',
        args: {},
        message: 'Service paused: The service is currently paused. Please try again later.',
        originalError: error,
      };
    }
    ...
    return {
      errorName: 'InternalJSONRPCError',
      args: { fullError: error },
      message:
        'The transaction failed. This might be due to incorrect parameters or insufficient permissions. Please check your DID authentication and try again.',
      originalError: error,
    };
  }
  ...
  protected handleRevertError(error: any): ParsedContractError {
    ...
    if (revertMatch?.[1]) {
      ...
      if (reason.toLowerCase().includes('did')) {
        return {
          errorName: 'DataRegistry__InvalidDID',
          args: {},
          message: 'Invalid DID: The DID does not exist or is not active',
          originalError: error,
        };
      }
