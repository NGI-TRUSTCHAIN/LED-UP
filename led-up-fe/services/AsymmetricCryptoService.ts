...
export class AsymmetricCryptoService {
  ...
  public encryptWithPublicKey(data: string, publicKey: string): string {
    try {
      ...
      const cipher = createCipheriv('aes-256-gcm', Buffer.from(sharedSecret, 'hex').subarray(0, 32), iv);
  ...
  public decryptWithPrivateKey(encryptedData: string, privateKey: string): string {
    try {
      ...
      const { ephemeralPublicKey, iv, authTag, encrypted } = JSON.parse(encryptedData) as AsymmetricEncryptOutput;
  ...
  public sign(data: string | Buffer, privateKey: string): string {
    try {
      ...
      if (this.curveType === CurveType.SECP256K1 && this.ecCurve) {
        ...
        const key = this.edCurve.keyFromSecret(privateKey);
        return key.sign(dataBuffer).toHex();
      } else {
        throw new Error('Invalid curve configuration');
      }
    } catch (error: any) {
  ...
  public verify(data: string | Buffer, signature: string, publicKey: string): boolean {
    try {
      ...
      if (this.curveType === CurveType.SECP256K1 && this.ecCurve) {
        ...
        // For ed25519, the keyFromPublic method takes only one argument
        const key = this.edCurve.keyFromPublic(Buffer.from(publicKey, 'hex'));
        // For ed25519, we need to convert the signature to a Buffer if it's a string
        const signatureBuffer = typeof signature === 'string' ? Buffer.from(signature, 'hex') : signature;
        return key.verify(dataBuffer, signatureBuffer);
      }
      return false;
    } catch (error) {
      return false;
  ...
  public encryptLargeData(
    ...
    try {
      ...
      const encryptedSymmetricKey = this.encryptWithPublicKey(symmetricKey.toString('hex'), publicKey);
   ...
   */
  public decryptLargeData(encryptedData: string, encryptedSymmetricKey: string, privateKey: string): string {
    try {
      if (this.curveType !== CurveType.SECP256K1) {
