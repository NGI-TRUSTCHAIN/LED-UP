...
export class CompensationService {
  ...
  async processPayment(params: ProcessPaymentParams): Promise<Record<string, any>> {
    try {
      const { producer, recordId, dataSize } = params;

      const tx = await this.contract.processPayment(producer, recordId, dataSize);
  ...
  /**
   * Gets the data registry address.
   * @returns A promise that resolves to the data registry address.
   */
  async getDataRegistryAddress(): Promise<string> {
    try {
      return await this.contract.getDataRegistryAddress();
    } catch (error) {
      throw new Error(this.errorHandler.handleError(error));
    }
  }
   ...
   * @returns A promise that resolves to the provider wallet address.
   */
  async getProviderWallet(): Promise<string> {
    try {
      return await this.contract.getLeveaWallet();
    } catch (error) {
      throw new Error(this.errorHandler.handleError(error));
    }
  }
  ...
  /**
   * Gets the provider wallet balance.
   * @returns A promise that resolves to the provider wallet balance.
   */
  async getProviderWalletBalance(): Promise<number> {
    try {
      return await this.contract.getLeveaWalletBalance();
    } catch (error) {
      throw new Error(this.errorHandler.handleError(error));
    }
  }
  ...
  /**
   * Registers a producer with their DID.
   * @param producer The address of the producer.
   * @param did The DID of the producer.
   * @returns A promise that resolves to the formatted transaction receipt.
   */
  async registerProducer(producer: string, did: string): Promise<Record<string, any>> {
    try {
      const tx = await this.contract.registerProducer(producer, did);
      const receipt = await tx.wait();
  ...
  /**
   * Registers a consumer with their DID.
   * @param consumer The address of the consumer.
   * @param did The DID of the consumer.
   * @returns A promise that resolves to the formatted transaction receipt.
   */
  async registerConsumer(consumer: string, did: string): Promise<Record<string, any>> {
    try {
      const tx = await this.contract.registerConsumer(consumer, did);
      const receipt = await tx.wait();
