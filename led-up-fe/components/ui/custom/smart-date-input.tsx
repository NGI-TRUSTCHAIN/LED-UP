 ...
 */
export const formatDateTime = (datetime: Date | string, locale: Locale, hour12: boolean) =>
 ...
 */
type SmartDatetimeInputProps = Omit<CalendarProps, 'id' | 'mode' | 'selected' | 'onSelect' | 'initialFocus'> & {
  value?: Date | null;
...
export const SmartDatetimeInput = React.forwardRef<
  HTMLInputElement,
  Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type' | 'ref' | 'value' | 'defaultValue' | 'onBlur' | 'disabled'> &
    ...
    return (
      <SmartDatetimeInputContext.Provider value={{ value, onValueChange, Time, onTimeChange }}>
        <div className="flex items-center justify-center">
          ...
          >
            <DateTimeLocalInput locale={locale} hour12={hour12} disabled={disabled} {...dateTimeInputProps} />
          ...
          {Array.from({ length: 24 }).map((_, i) => {
            ...
            return Array.from({ length: 4 }).map((_, part) => {
              ...
              const currentValue = !hour12
                ...
                : `${formatIndex}:${part === 0 ? '00' : timestamp * part} ${PM_AM}`;
    ...
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (parsedDateTime) {
        if (hour12) {
          ...
          onValueChange(parsedDateTime);
          setInputValue(formatDateTime(parsedDateTime, locale, hour12));
          onTimeChange(`${parsedDateTime.getHours()}:${parsedDateTime.getMinutes()}`);
        }
      }
    },
...
const DateTimeLocalInput = ({ locale, hour12, className, ...props }: DateTimeLocalInputProps) => {
