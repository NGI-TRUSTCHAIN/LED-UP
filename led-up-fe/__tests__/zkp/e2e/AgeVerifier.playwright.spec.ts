...
test.describe('Age Verifier', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Wait for the Age tab content to be active
    await page.waitForSelector('[data-state="active"][role="tabpanel"]');
  });

  test('should render Age Verifier tab', async ({ page }) => {
    // Check that the Age title is visible - use a more specific selector
    // to target only the main heading, not any other elements that might contain the text
  ...
  test('should switch between tabs', async ({ page }) => {
    ...
    // Click back to Age Verifier tab
  ...
  test.skip('should take a visual snapshot', async ({ page }) => {
    ...
    await expect(tabPanel).toHaveScreenshot('age-verifier.png');
  });
});
